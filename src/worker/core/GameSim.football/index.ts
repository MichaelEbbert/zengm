...
class GameSim extends GameSimBase {
	...
	async simPlay() {
		...
		this.playCount += 1;
		toUI("updateLocal", [{ liveGameProgress: this.playCount * 3 }]);
	}
	...
	doPunt() {
		...
		if (this.scrimmage + averageDistance >= 90) {
			...
			distance = Math.round(random.truncGauss(averageDistance, sigma, 25, 90));
		}
